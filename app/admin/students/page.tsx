...
export default function AdminStudentsRosterPage() {
  ...
  const toggleUserStatus = async (user: User) => {
    try {
      await userService.updateUserStatus(user._id, { is_active: !user.is_active });
  ...
  const totalWalletBalance = students.reduce((sum, s) => sum + (s.wallet_balance || 0), 0);
                ...
                {students.map((student) => (
                  <tr key={student._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 px-6 flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="font-bold text-slate-900 text-sm leading-tight">{student.fullName}</p>
                    ...
                    <td className="py-4 px-6 font-bold text-emerald-600">{formatCurrency(student.wallet_balance || 0)}</td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${student.is_active ? "bg-emerald-500" : "bg-[#D00113]"}`} />
                    ...
                    <td className="py-4 px-6 text-right space-x-3">
                      <button 
                        ...
                        className={`text-xs font-bold transition-colors ${student.is_active ? 'text-amber-600 hover:text-amber-700' : 'text-emerald-600 hover:text-emerald-700'}`}
                      >
