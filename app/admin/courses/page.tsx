...
export default function AdminCoursesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (coursesRes.success) setCourses(coursesRes.data.data || coursesRes.data);
      if (catsRes.success) setCategories(catsRes.data.data || catsRes.data);
    } catch (error) {
  ...
  const handleEdit = (course: Course) => {
    setFormData({
      ...
      access_type: course.pricing?.type === "FREE" ? "free" : "paid",
              ...
              {courses.map((course) => (
                <tr key={course._id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="py-4 px-6 font-bold text-slate-800">
                    {course.pricing?.type === "FREE" ? "Free" : formatCurrency(course.pricing?.price || 0)}
                  </td>
                  <td className="py-4 px-6">
